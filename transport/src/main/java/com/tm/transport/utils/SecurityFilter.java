package com.tm.transport.utils;
...
import com.tm.transport.model.UserView;
...
public class SecurityFilter implements ContainerRequestFilter{
	...
	private static final String SECURED_URI_PREFIX = "securedtransport";
	...
	public void filter(ContainerRequestContext requestContext) throws IOException {
		if(requestContext.getUriInfo().getPath().contains(SECURED_URI_PREFIX)) {
			...
			if(authHeader!=null && !authHeader.isEmpty()) {
				...
				beginTransaction();
				UserView user = entityManager.find(UserView.class, userName);
				endTransaction();
